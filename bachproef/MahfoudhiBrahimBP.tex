...
\title[]%
    {Validatie van een IoT-systeem voor realtime monitoring van wachtruimtebezetting: Een vergelijking met Google Occupancy Analytics}
...
\begin{listing}[H]
    ...
    \begin{minted}{sql}
        SELECT
        TIMESTAMP_TRUNC(
        TIMESTAMP(SUBSTR(JSON_VALUE(annotation, "$.currentTime"), 1, 26)), MINUTE
        ) AS currentTimeUTC,
        MAX(CAST(JSON_VALUE(count_obj, "$.count") AS INT64)) AS people_in_zone
        FROM
